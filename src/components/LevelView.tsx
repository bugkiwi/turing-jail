...
export function LevelView({ locale, playerId, level, question, attempt, startedAt, draft, onDraftChange, onResult, onBoard, audioOn }: Props) {
  ...
  const [preview, setPreview] = useState<Evaluation>({ noul: 0, persuasiveness: 0, tactic: 'other', plea: 0, logic: 0, paradox: 0 });
  ...
  function onInput(value: string) {
    ...
    if (value.trim().length === 0) {
      setPreview({ noul: 0, persuasiveness: 0, tactic: 'other', plea: 0, logic: 0, paradox: 0 });
      setPhase('idle');
      return;
    }
    setPhase('idle');
  ...
  function quickScore(score: number) {
    if (!text.trim() || phase === 'idle' || phase === 'evaluating') return <span className="stitch-live-placeholder">{text.trim() ? '…' : '—.—'}</span>;
